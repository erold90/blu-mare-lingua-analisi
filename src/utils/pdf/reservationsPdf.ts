...
export const generateReservationsPdf = (reservations: Reservation[], apartments: Apartment[]) => {
  ...
  // Generate PDF content for each apartment
  Object.keys(reservationsByApartment).forEach((apartmentId, apartmentIndex) => {
    ...
    if (apartmentReservations.length === 0) {
      ...
      // Reservation rows
      apartmentReservations.forEach((reservation, index) => {
        // Check if we need a new page
        if (yPosition > 270) {
          doc.addPage();
          yPosition = 20;
          
          // Repeat apartment header on new page
          doc.setFont('helvetica', 'bold');
          doc.setFontSize(14);
    ...
    yPosition += 10; // Space between apartments
  });
