-- Fase 2: Sistema RLS migliorato per tutte le tabelle esistenti
...
-- 7. ANALYTICS_SUMMARY TABLE - Solo admin può vedere
ALTER TABLE public.analytics_summary ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Admin can view analytics" ON public.analytics_summary
  FOR SELECT USING (public.has_role(auth.uid(), 'admin'::app_role));

-- 8. SITE_VISITS TABLE - Manteniamo le policy esistenti ma aggiungiamo controllo admin
