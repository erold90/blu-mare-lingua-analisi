-- Correggi il mapping delle settimane per data invece che per numero
-- Cancella tutti i prezzi 2026-2035 e rigenera con mapping corretto per data

DELETE FROM weekly_prices WHERE year BETWEEN 2026 AND 2035;

-- Crea una funzione che mappa correttamente le settimane per data
CREATE OR REPLACE FUNCTION generate_weekly_prices_by_date_mapping(target_year integer, copy_from_year integer DEFAULT NULL)
RETURNS integer
LANGUAGE plpgsql
AS $$
DECLARE
  ...
  FOR apartment_record IN SELECT 'appartamento-' || id::text as apartment_id FROM public.apartments LOOP
    ...
    WHILE current_week_start <= season_end LOOP
      ...
      BEGIN
        ...
        -- Map to corresponding date in copy_from_year (same month/day, different year)
        IF copy_from_year IS NOT NULL THEN
          corresponding_date := make_date(copy_from_year, EXTRACT(MONTH FROM current_week_start)::integer, EXTRACT(DAY FROM current_week_start)::integer);
          
          -- Find a price record that covers this corresponding date
          ...
          WHERE apartment_id = apartment_record.apartment_id 
            ...
            AND week_start <= corresponding_date
            AND week_end >= corresponding_date
          LIMIT 1;
          
          -- If no exact match, try finding the closest week
          IF price_to_use IS NULL THEN
            SELECT price INTO price_to_use
            FROM public.weekly_prices 
            WHERE apartment_id = apartment_record.apartment_id 
              AND year = copy_from_year 
            ORDER BY ABS(EXTRACT(EPOCH FROM (week_start - corresponding_date)))
            LIMIT 1;
          END IF;
        END IF;
        
        -- If no specific price found for this apartment, use defaults based on week position
        IF price_to_use IS NULL THEN
          CASE 
...
-- Genera prezzi per tutti gli anni 2026-2035 con mapping corretto per data
SELECT generate_weekly_prices_by_date_mapping(2026, 2025);
SELECT generate_weekly_prices_by_date_mapping(2027, 2025);
SELECT generate_weekly_prices_by_date_mapping(2028, 2025);
SELECT generate_weekly_prices_by_date_mapping(2029, 2025);
SELECT generate_weekly_prices_by_date_mapping(2030, 2025);
SELECT generate_weekly_prices_by_date_mapping(2031, 2025);
SELECT generate_weekly_prices_by_date_mapping(2032, 2025);
SELECT generate_weekly_prices_by_date_mapping(2033, 2025);
SELECT generate_weekly_prices_by_date_mapping(2034, 2025);
SELECT generate_weekly_prices_by_date_mapping(2035, 2025);